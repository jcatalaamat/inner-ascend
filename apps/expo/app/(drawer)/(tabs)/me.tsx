import { ScrollView, Text, Card, YStack, XStack, Spinner, Button, Avatar, getTokens } from '@my/ui'
...
import { useUpcomingSessionsQuery, useUserRsvpsQuery } from 'app/utils/react-query/useLiveSessionsQuery'
import { FloatingMenuButton } from 'app/components/FloatingMenuButton'
import { useUser } from 'app/utils/useUser'
import { Settings, Pencil } from '@tamagui/lucide-icons'
...
export default function MeScreen() {
  ...
  const { profile, displayName, avatarUrl } = useUser()
  ...
  // Get upcoming sessions with user RSVPs
  const rsvpMap = new Map(userRsvps?.map((rsvp) => [rsvp.session_id, rsvp.rsvp_status]))
  const confirmedSessions = upcomingSessions?.filter((session) => rsvpMap.get(session.id) === 'yes') || []
  ...
  return (
    <YStack flex={1} backgroundColor="$deepSpace1">
      ...
      >
        {/* Profile Header */}
        <Card padding="$5" marginBottom="$4" backgroundColor="$deepSpace2">
          <XStack justifyContent="space-between" alignItems="center">
            <XStack gap="$4" alignItems="center" flex={1}>
              <Avatar circular size="$6">
                ...
                <Avatar.Fallback bc="$cosmicViolet" ai="center" jc="center">
                  <Text fontSize="$8" color="$silverMoon" fontWeight="bold">
            ...
            <XStack gap="$2">
              <Button
                ...
                icon={<Pencil size={18} color="$silverMoon2" />}
              />
              <Button
                size="$3"
                backgroundColor="$deepSpace3"
                ...
                icon={<Settings size={18} color="$silverMoon2" />}
              />
            </XStack>
          </XStack>
        </Card>
        ...
        {/* Community Section */}
        <Text fontSize="$7" fontWeight="bold" color="$silverMoon" marginTop="$4" marginBottom="$3">
          COMMUNITY
        </Text>
        {confirmedSessions.length > 0 ? (
            ...
            <XStack justifyContent="space-between" alignItems="center" marginBottom="$3">
              ...
              <Card padding="$2" paddingHorizontal="$3" backgroundColor="$cosmicViolet">
                <Text fontSize="$2" color="$silverMoon" fontWeight="600">
                  {confirmedSessions.length} Coming Up
                </Text>
              </Card>
            </XStack>
            <YStack gap="$2">
              {confirmedSessions.slice(0, 2).map((session) => {
                ...
                return (
                  <Card
                    ...
                    onPress={() => router.push('/(drawer)/(tabs)/community')}
                  >
                    <Text fontSize="$4" color="$cosmicViolet" fontWeight="600" marginBottom="$1">
                      {dateStr}
                    </Text>
                    <Text fontSize="$4" color="$silverMoon" numberOfLines={1}>
                      {session.title}
                    </Text>
                  </Card>
                )
          ...
          <Card padding="$4" backgroundColor="$deepSpace2" marginBottom="$4">
            ...
            <Text fontSize="$3" color="$silverMoon2" marginBottom="$3">
              {nextSession.title}
            </Text>
            <Button
              size="$4"
          ...
          <Card padding="$4" backgroundColor="$deepSpace2" marginBottom="$4">
            ...
            <Text color="$silverMoon2" fontSize="$3" marginBottom="$3">
              Join upcoming healing circles and workshops
            </Text>
            <Button
              size="$4"
              theme="alt2"
              onPress={() => router.push('/(drawer)/(tabs)/community')}
            >
              <Text>Explore Sessions →</Text>
            </Button>
          </Card>
        )}

        {/* Journal Entries Section */}
        <Text fontSize="$7" fontWeight="bold" color="$silverMoon" marginBottom="$3">
          JOURNAL
        </Text>
        <Card padding="$4" backgroundColor="$deepSpace2" marginBottom="$4">
          <XStack justifyContent="space-between" alignItems="center" marginBottom="$3">
